...
- Platform API client, including browser-based OIDC login (RFC 8252 loopback
  ...
  TTL) is re-exchanged against the same code first — a wrong-replica miss never
  consumes it, so this rides the load balancer to the issuing replica with no
  new browser round trip — and then restarted with fresh verifier/state/code,
  up to three attempts (`kwclient.nativeLoginAttempts`), matching the API's own
